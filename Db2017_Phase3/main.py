...
from handler.user_login import userLoginHandler
...
# Buy a resource
@app.route('/DisasterApp/purchase/buy_resource', methods=['POST'])
def insertPurchase():
...
@app.route('/DisasterApp/resources/inserted', methods=['GET', 'POST'])
def getResourceInserted():
    if request.method == 'POST':
        ...
        if not request.args:
            ...
            return resourcesHandler().searchResourceInserted(request.args)
...
# Insert free resource
@app.route('/DisasterApp/resources/free_resource', methods=['POST'])
def insertFreeResource():
    return resourcesHandler().insertFreeResource(request.form)
...
def getRequestDetailsByRDid(RDid):
    if request.method == 'GET':
        ...
        return requestDetailsHandler().updateRequestDetails(RDid, request.form)
    else:
...
# Ver transaccion de compra de un recurso
@app.route('/DisasterApp/request_details/purchase/<int:pid>', methods=['GET'])
def getPaymentOfResource(pid):
    return requestDetailsHandler().getPaymentOfResource(pid)
###########################################################################################
...
@app.route('/DisasterApp/creditCard/<int:uid>', methods=['PUT'])
def getCreditCardByUid(uid):
    if request.method == 'PUT':
        return creditCardHandler().updateCreditCard(uid, request.form)
...
@app.route('/DisasterApp/Does', methods=['GET'])
def getAllDoes():
    if not request.args:
...
@app.route('/DisasterApp/Sales', methods=['GET'])
def getAllSales():
    if not request.args:
...
# Get Sales by sid
@app.route('/DisasterApp/Sales/<int:sid>', methods=['GET'])
def getSalesBysid(sid):
    return SalesHandler().getSalesBysid(sid)
####################################################################################

###################All possibilities for User_Login#################################

@app.route('/DisasterApp/user_login', methods=['GET', 'POST'])
def getAllUserLogin():
    if request.method == 'POST':
        return userHandler().insertUser(request.form)
    else:
        if not request.args:
            return userLoginHandler().getAllUserLogin()
        else:
            return userLoginHandler().searchUserLogin(request.args)

@app.route('/DisasterApp/user_login/users/<int:uid>', methods=['GET'])
def getUserLoginByUid(uid):
    return userLoginHandler().getUserLoginByUid(uid)
