...
class resourceLocationDAO:
    ...
    ##################### POST METHODS ##########################################
    def insert(self, rcity, rregion, rid):
        cursor = self.conn.cursor()
        query = "insert into resource_location(rcity, rregion, rid) values (%s, %s, %s) returning rlid;"
        cursor.execute(query, (rcity, rregion, rid,))
        rlid = cursor.fetchone()[0]
        self.conn.commit()
        return rlid
