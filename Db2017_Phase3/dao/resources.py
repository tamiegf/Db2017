...
#############
    ...
    def insertFreeResource(self, user_name, user_password, rname, rqty, cname, cdescription,  rcity, rregion):
        rprice = 0
        cursor = self.conn.cursor()
        ...
        if utype != "supplier":
            ...
            query3 = "insert into resources(rname, rqty, rprice, uid) values (%s, %s, %s, %s) returning rid;"
            cursor.execute(query3, (rname, rqty, rprice, uid, ))
            rid = cursor.fetchone()[0]
            query4 = "insert into resource_category(cname, cdescription, rid) values (%s, %s, %s);"
            cursor.execute(query4, (cname, cdescription, rid, ))
            query5 = "insert into resource_location(rcity, rregion, rid) values (%s, %s, %s);"
            cursor.execute(query5, (rcity, rregion, rid, ))
            return rid

    def insert(self, rname, rqty, rprice, rdid, uid):
        cursor = self.conn.cursor()
        if rdid == '':
            query = "insert into resources(rname, rqty, rprice, uid) values (%s, %s, %s, %s) returning rid;"
            cursor.execute(query, (rname, rqty, rprice, uid,))
        else:
            query = "insert into resources(rname, rqty, rprice, uid, rdid) values (%s, %s, %s, %s, %s) returning rid;"
            cursor.execute(query, (rname, rqty, rprice, uid, rdid,))
        rid = cursor.fetchone()[0]
        self.conn.commit()
        return rid
    ...
    def updateall(self, rid, rname, rprice, rqty, rdid, uid):
        cursor = self.conn.cursor()
        query = "update resources set rname=%s, rqty=%s, rprice=%s, uid=%s, rdid=%s where rid=%s;"
        cursor.execute(query, (rname, rqty, rprice, uid, rdid, rid,))
        self.conn.commit()
        return rid

    def updaterpriceAndrqty(self, rid, rprice, rqty):
        cursor = self.conn.cursor()
        query = "update resources set rqty=%s, rprice=%s where rid=%s returning rname, uid, rdid;"
        cursor.execute(query, (rqty, rprice, rid,))
        result = []
        for row in cursor:
            result.append(row)
        self.conn.commit()
    ...
    def updaterprice(self, rid, rprice):
        cursor = self.conn.cursor()
        query = "update resources set rprice=%s where rid=%s returning rname, uid, rdid, rqty;"
        cursor.execute(query, (rprice, rid,))
        result = []
        for row in cursor:
            result.append(row)
        self.conn.commit()
    ...
    def updaterqty(self, rid, rqty):
        cursor = self.conn.cursor()
        query = "update resources set rqty=%s where rid=%s returning rname, uid, rdid, rprice;"
        cursor.execute(query, (rqty, rid,))
        result = []
        for row in cursor:
            result.append(row)
        self.conn.commit()
    ...
    def updaterdid(self, rid, rdid):
        cursor = self.conn.cursor()
        query = "update resources set rdid=%s where rid=%s returning rname, uid, rqty, rprice;"
        cursor.execute(query, (rdid, rid,))
        result = []
        for row in cursor:
            result.append(row)
        self.conn.commit()
    ...
    def updatername(self, rid, rname):
        cursor = self.conn.cursor()
        query = "update resources set rname=%s where rid=%s returning rdid, uid, rqty, rprice;"
    ...
    def updateuid(self, rid, uid):
        cursor = self.conn.cursor()
        query = "update resources set uid=%s where rid=%s returning rdid, rname, rqty, rprice;"
